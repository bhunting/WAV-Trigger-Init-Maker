...
void MainComponent::reset(void)
{
	...
	mInitStrings.add("This file was generated by the WAV Trigger Init File Maker v1.10");
	mInitStrings.add(newLine);
	...
	if ((functionBox->getSelectedId() > 1) && (functionBox->getSelectedId() < 8)) {
		...
		if (tStr.length() == 0) {
			AlertWindow::showMessageBoxAsync(AlertWindow::InfoIcon, "Track Range Error",
				"Track Range values for this Trigger Function type must be set to 1 - 2048, with HIGH >= LOW");
			return tStr;
		}
	}
	else {
...
StringArray tokens;
	...
	// Read and set the retrigger and loop toggles
	if (n > 4) {
...
int val;
	...
	for (int n = 0; n < mInitStrings.size(); n++) {
		if (mInitStrings[n].startsWith("#MIDI")) {
			...
			if (val & 0x01) {
				baudBox->setSelectedId(7, dontSendNotification);
				if (val & 0x02)
	...
	for (int n = 0; n < mInitStrings.size(); n++) {
		if (mInitStrings[n].startsWith("#VOLM")) {
			...
			line = line.trimStart();
			val = line.getIntValue();
			volSlider->setValue((double)val, dontSendNotification);
		}
	}
	...
	for (int n = 0; n < mInitStrings.size(); n++) {
		if (mInitStrings[n].startsWith("#LPWR")) {
			...
			line = line.trimStart();
			val = line.getIntValue();
			lpToggle->setToggleState(true, dontSendNotification);
			sleepBox->setEnabled(true);
			sleepText->setEnabled(true);
			switch (val)	{
				case 15:
					sleepBox->setSelectedId(1, dontSendNotification);
				break;
				case 30:
					sleepBox->setSelectedId(2, dontSendNotification);
				break;
				case 60:
					sleepBox->setSelectedId(3, dontSendNotification);
				break;
				case 120:
					sleepBox->setSelectedId(4, dontSendNotification);
				break;
				case 180:
					sleepBox->setSelectedId(5, dontSendNotification);
				break;
				case 240:
					sleepBox->setSelectedId(6, dontSendNotification);
				break;
				case 300:
					sleepBox->setSelectedId(7, dontSendNotification);
				break;
				case 600:
					sleepBox->setSelectedId(8, dontSendNotification);
				break;
				case 900:
					sleepBox->setSelectedId(9, dontSendNotification);
				break;
				case 1800:
					sleepBox->setSelectedId(10, dontSendNotification);
				break;
				case 2700:
					sleepBox->setSelectedId(11, dontSendNotification);
				break;
				case 3600:
					sleepBox->setSelectedId(12, dontSendNotification);
				break;
			}
		}
	}
